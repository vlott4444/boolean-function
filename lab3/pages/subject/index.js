...
export class SubjectPage {
    ...
    // === ИЗМЕНЁННЫЙ HTML (добавлена колонка для 3D) ===
    getHTML() {
        return `
    ...
    clickBack() {
        // === ОЧИСТКА 3D ПРИ УХОДЕ ===
        this.dispose3D();
        const mainPage = new MainPage(this.parent);
        mainPage.render();
    }

    // === НОВЫЙ МЕТОД: ИНИЦИАЛИЗАЦИЯ 3D ===
    async init3D() {
    ...
    // 2. Камера (временно, будет скорректирована после загрузки модели)
    ...
    // 4. Управление (target будет установлен после загрузки)
    ...
    // 6. Вспомогательная сетка (пол)
    const gridHelper = new THREE.GridHelper(10, 20, 0xDB3F59, 0xDB3F59);
    gridHelper.position.y = -0.8;
    this.scene.add(gridHelper);

    // 7. Загрузка модели .glb с автоматическим масштабированием
    ...
    try {
        ...
        // ---- МАСШТАБИРОВАНИЕ ----
        // Вычисляем оригинальный bounding box
        const box = new THREE.Box3().setFromObject(this.model);
        const size = box.getSize(new THREE.Vector3());
        const maxDim = Math.max(size.x, size.y, size.z);
        // Целевой размер модели по наибольшей оси (подберите под свои предпочтения)
        const targetSize = 1.5;
        const scale = targetSize / maxDim;
        this.model.scale.set(scale, scale, scale);

        // После масштабирования пересчитываем box для позиционирования на пол
        const scaledBox = new THREE.Box3().setFromObject(this.model);
        const minY = scaledBox.min.y;
        this.model.position.y -= minY; // ставим на уровень пола (y=0)

        this.scene.add(this.model);

        // ---- НАСТРОЙКА КАМЕРЫ ПОД РАЗМЕР МОДЕЛИ ----
        // Центр модели (приблизительно на половине высоты)
        const modelCenterY = (scaledBox.max.y - scaledBox.min.y) / 2;
        this.controls.target.set(0, modelCenterY, 0);
        // Устанавливаем камеру на расстоянии, пропорциональном размеру модели
